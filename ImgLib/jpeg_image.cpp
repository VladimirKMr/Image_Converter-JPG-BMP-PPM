...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    const Color* start = image.GetLine(y);
    const Color* end = start + image.GetWidth();

    int index = 0;

    for (;start < end; ++start) {
        buffer[index] = static_cast<JSAMPLE>(start->r);
        buffer[index + 1] = static_cast<JSAMPLE>(start->g);
        buffer[index + 2] = static_cast<JSAMPLE>(start->b);
